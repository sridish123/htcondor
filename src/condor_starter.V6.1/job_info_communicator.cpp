...
JobInfoCommunicator::~JobInfoCommunicator()
{
	...
	if( job_credential_file ) {
		free( job_credential_file );
	}
#if HAVE_JOB_HOOKS
    if (m_hook_mgr) {
        delete m_hook_mgr;
...
void
JobInfoCommunicator::set_credential_file( const char* path )
{
	if( job_credential_file ) {
		free( job_credential_file );
	}
	job_credential_file = strdup( path );
}
